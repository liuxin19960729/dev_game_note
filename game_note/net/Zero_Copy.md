...
1.减少甚至完全避免操作系统内核和用户应用程序地址空间这两者之间进行数据拷贝操作，从而减少用户态 -- 内核态上下文切换带来的系统开销。

2.减少甚至完全避免操作系统内核缓冲区之间进行数据拷贝操作。

3.帮助用户进程绕开操作系统内核空间直接访问硬件存储接口操作数据。

4.利用 DMA 而非 CPU 来完成硬件接口和内核缓冲区之间的数据拷贝，从而解放 CPU，使之能去执行其他的任务，提升系统性能。
...
##  Zero-copy 的实现方式有哪些？
![Linux Zero copy](./imgs/l_zero_c.jpg)
```
linux 实现零拷贝的方式
核心思想大概有3大类

1.减少甚至避免用户空间和内核空间之间的数据拷贝
    在一些数据的传输过程中并不需要对数据访问所以linux直接让数据缓存在内核中，不让他拷贝的用户态，这类操作有专门系统调用api来完成比如 linux mmap() sendfile() splice();
2.绕过内核的直接 I/O
    用户直接访问硬件不通过内核进行数据的拷贝，内核只做一些辅助和管理的工作 。
...
优点
1.一是节省内存空间，因为用户进程上的这一段内存是虚拟的，并不真正占据物理内存，只是映射到文件所在的内核缓冲区上，因此可以节省一半的内存占用
2.省去了一次 CPU 拷贝，对比传统的 Linux I/O 读写，数据不需要再经过用户进程进行转发了，而是直接在内核里就完成了拷贝

    mmap() 3次拷贝 用户态-内核态的转换4次


适合场景
mmap() 因为既节省 CPU 拷贝次数又节省内存，所以比较适合大文件传输的场景


缺点
    它并不总是能达到理想的数据传输性能
    因为数据数据传输过程中依然需要一次 CPU 拷贝(磁盘控制缓存到内核缓存数据的拷贝)
    内存映射技术是一个开销很大的虚拟存储操作：这种操作需要修改页表以及用内核缓冲区里的文件数据汰换掉当前 TLB 里的缓存以维持虚拟内存映射的一致性

    为什么mmap()适合较大的文件的传输。
    因为内存映射通常针对的是相对较大的数据区域，所以对于相同大小的数据来说，内存映射所带来的开销远远低于 CPU 拷贝所带来的开销


mmap()-->write()需要关注的问题

例如，在 mmap() --> write() 这两个系统调用的整个传输过程中，如果有其他的进程突然截断了这个文件，那么这时用户进程就会因为访问非法地址而被一个从总线传来的 SIGBUS 中断信号杀死并且产生一个 core dump
